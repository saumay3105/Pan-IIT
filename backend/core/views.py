...
def send_email_with_video(request):
    ...
    try:
        ...
        if not job_id:
            return JsonResponse({"error": "Missing job_id in request body."}, status=400)

        # Construct the video path dynamically
        video_file_path = os.path.join(os.path.dirname(__file__), f"../media/generated_videos/{job_id}.mp4")
